...
void eeprom_init(bool _reset)
{
  ...
  if (read_eeprom_8(EEPROM_INDEX_FOR_LB_SHUTDOWN_DELAY) > MAX_THRESH_DELAY ||
      read_eeprom_8(EEPROM_INDEX_FOR_LB_SHUTDOWN_DELAY) < MIN_THRESH_DELAY || _reset)
  {
    write_eeprom_8(EEPROM_INDEX_FOR_LB_SHUTDOWN_DELAY, DEFAULT_THRESH_DELAY);
  }
