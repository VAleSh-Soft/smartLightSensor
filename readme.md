...
Кроме того здесь расположен бегунок регулировки яркости индикаторного светодиода. Диапазон регулировки - **10%..100%**. Заданный коэффициент применяется как минимальному (`MIN_LED_BRIGHTNESS`), так и к максимальному (`MAX_LED_BRIGHTNESS`) уровням яркости.
